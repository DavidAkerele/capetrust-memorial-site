...
import { Mail, MapPin, Phone, Calculator, Flame, Shield } from "lucide-react";
          ...
          <div className="flex items-center gap-3.5">
            ...
            <div>
              <p className="font-serif text-2xl font-bold tracking-tight text-white">
                {currentSettings.shortName || "Capetrust"}
              </p>
              <p className="eyebrow text-gold text-[0.6rem]">Funeral Services</p>
            </div>
          </div>
          <p className="max-w-sm text-xs leading-relaxed text-white/80">
            {currentSettings.tagline || site.tagline} Providing compassionate funeral planning, dignified burials, and serene resting grounds at Garden of Peace™ Memorial Park, Agbowa, Lagos.
          </p>
          <div className="pt-2">
            <span className="inline-flex items-center rounded-full bg-[#1E3F20] px-3 py-1 text-xs font-semibold text-white border border-[#D4AF37]/30">
              ...
              <a
                ...
                className="ml-1 hover:underline text-gold font-bold"
              >
                {currentSettings.phone || site.phone}
              </a>
            </span>
          </div>
        </div>
        ...
        <div className="md:col-span-3">
          ...
          <ul className="mt-4 space-y-3 text-xs text-primary-foreground/85">
            <li className="flex gap-2">
              ...
              <a href={currentSettings.phoneHref || site.phoneHref} className="hover:underline">
                {currentSettings.phone || site.phone}
              </a>
            </li>
            <li className="flex gap-2">
              <Mail className="mt-0.5 size-3.5 text-gold shrink-0" />
              <a href={`mailto:${currentSettings.email || site.email}`} className="hover:underline">
                {currentSettings.email || site.email}
              </a>
            </li>
            <li className="flex gap-2">
              <MapPin className="mt-0.5 size-3.5 text-gold shrink-0" />
              <span>
                <strong>Head Office:</strong> {currentSettings.headOffice || site.headOffice}
              </span>
            </li>
            <li className="flex gap-2">
              <MapPin className="mt-0.5 size-3.5 text-gold shrink-0" />
              <span>
                <strong>Memorial Park:</strong> {currentSettings.park || site.park}
              </span>
            </li>
          </ul>
        </div>
      ...
      <div className="border-t border-primary-foreground/15">
        <div className="container-page flex flex-col gap-2 py-5 text-xs text-primary-foreground/60 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex items-center gap-4">
            <span>Garden of Peace™ Memorial Park · Agbowa-Ikosi, Lagos</span>
            <span>•</span>
            <Link
              to="/admin"
              className="inline-flex items-center gap-1 text-primary-foreground/40 transition-colors hover:text-gold"
            >
